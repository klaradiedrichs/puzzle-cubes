...
const PhasenItem = ({ index, id, title, nextAccordion }: Props) => {
  ...
  const { appState, addAppState, updateAppState, existsAppState } =
    usePerformanceBotStore();
  ...
  return (
    <div key={id} className="">
      ...
      <div
        ...
        `}
      >
        <p className={` text-6xl font-martin tracking-wide uppercase `}>
          {title}
        </p>
      </div>

      {/* Beschreibung */}
      <div className="absolute left-[194px] top-0 bottom-0">
        <div
          className={`${
            isActive === true ? "flex" : "hidden"
          } text-black bg-white border-8 border-black px-9 py-6 flex-wrap h-[370px] w-full justify-center`}
        >
          <ImageGallery />
        </div>
      </div>
